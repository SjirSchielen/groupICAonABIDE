function path = obtainFilePath(pathIn, prepstep, outdir, realignmentFirst)
...
%           - 9 band-pass filter
    ...
    switch prepstep
        case {3, 'coregistration', 'Coregistration', ...
            ...
            toCheck = [toCheck(1:anatind(end-1)) 'rest_1' toCheck(anatind(end):end)]; %replace with rest_1
    ...
    path = toCheck;
    if ~isfile(toCheck)
        error(sprintf("File path does not lead to a file. File path: %s", toCheck)); %#ok<SPERR>
    end
end